#!/bin/bash
# ----------------------------------------------------------------------------------
# @name    : advanced dpkg-scanpackages (versi langsung)
# @version : 1.1.3
# @date    : 03/04/2013 08:21:20
# 
# TENTANG
# ----------------------------------------------------------------------------------
...
# PERINGATAN..!
# ----------------------------------------------------------------------------------
# [!] Script ini adalah versi langsung/tanpa meminta konfirmasi di setiap prosesnya.
#     Untuk versi yang dilengkapi dengan konfirmasi, gunakan script dengan nama
#     'advanced-dpkg-scanpackages' (tanpa ada akhiran 'lanngsung')
...
# Override $HOME
# karena pada saat di-run dengan crontab root, user dianggap sebagai root
HOME="/home/goji"
...
mulai() {
   ...
   if [ "$mode" = "backup" -o "$mode" = "hapus" ]; then
      ...
      escfolder=$escfolder"\/"
      jumdeb=$(ls $lokasi/$namafolder/*.deb | wc -l)
...
do_cekcache() {
   ...
   cekjumlah=$(ls | wc -l)
   tglbackupchache=$(date +'%d %b %Y %H:%M')
   if [ "$cekjumlah" = "2" ]; then
      ...
      echo "Cache kosong - Auto Backup sebesar $cektotal - $tglbackupchache" > /home/goji/conkystat/cachewatch
      chown goji:goji /home/goji/conkystat/cachewatch
   fi
}
...
tanggal() {
	echo $tglscan > $HOME/conkystat/lastscan
}
